...
A = @(x) BsplineGeometry(x,'A',knots,coefs);
dAdx = @(x) BsplineGeometry(x,'dAdx',knots,coefs);
D = @(x) BsplineGeometry(x,'D',knots,coefs);
...
% Test throat finding function
[xThroat, yThroat] = BsplineGeometry(0,'throat',knots,coefs);
fprintf('Throat at x = %f with value of %f\n',xThroat,yThroat);
