...
class Environment:
    def __init__(self, altitude, hInf):
        ...
        self.P = P # Pa, atmospheric pressure
        self.T = T # K, atmospheric temperature
        self.rho = rho # kg/m^3, atmospheric density
        self.mu = mu # N-s/m^2, dynamic viscosity
        self.c = c # m/s, speed of sound
...
def StndAtm(h):
    ...
    rho = RHOSL*saSigma
    P = PRESSSL*saDelta
    mu = 0.0226968*T**1.5 / (T+198.72) / 1000000.0
    c = np.sqrt( 1.4*1716.56*T )
